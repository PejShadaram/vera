    ...
    <div className="space-y-6">
      ...
      {cases.length === 0 ? (
        <div className="border-2 border-dashed border-gray-200 rounded-2xl p-16 text-center">
          ...
          <p className="text-gray-500 mb-6 text-sm">Start by creating your first case. Vera will help you organize everything.</p>
          <Link href="/cases/new"
            className="bg-gray-900 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-gray-700 transition-colors text-sm">
            + Start a new case
          </Link>
        </div>
      ) : (
            ...
            <Link key={c.id as string} href={`/cases/${c.id}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="font-semibold text-gray-900 group-hover:text-gray-700 truncate">{c.name as string}</p>
