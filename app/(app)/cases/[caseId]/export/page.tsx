...
export default async function ExportPage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  return (
    <div style={{ fontFamily: "Georgia, 'Times New Roman', serif", maxWidth: 800, margin: "0 auto", padding: "40px 32px", color: "#1C1917", lineHeight: 1.6 }}>

      {/* Print button — hidden when printing */}
      <div className="print:hidden" style={{ marginBottom: 32, display: "flex", gap: 12 }}>
        <button onClick={() => window.print()}
          style={{ background: "#C2853A", color: "#fff", border: "none", padding: "10px 24px", borderRadius: 8, fontFamily: "inherit", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          Print / Save as PDF
        </button>
        <a href={`/cases/${caseId}`}
          style={{ border: "1px solid #E8E2D9", color: "#78716C", padding: "10px 24px", borderRadius: 8, fontFamily: "inherit", fontSize: 14, fontWeight: 500, textDecoration: "none", display: "inline-block" }}>
          ← Back to case
        </a>
      </div>
      ...
      <div style={{ borderBottom: "2px solid #1C1917", paddingBottom: 16, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <p style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.15em", color: "#78716C", margin: "0 0 4px" }}>Case File — Vera</p>
            <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>{c.name as string}</h1>
          </div>
          <div style={{ textAlign: "right", fontSize: 11, color: "#78716C" }}>
            <div>Generated {generated}</div>
            <div style={{ marginTop: 4, fontSize: 10, color: "#A8A29E" }}>Not legal advice</div>
          </div>
        </div>
        <div style={{ marginTop: 12, display: "flex", gap: 24, flexWrap: "wrap", fontSize: 13, color: "#78716C" }}>
          {c.opposing_party && <span><strong style={{ color: "#1C1917" }}>vs.</strong> {c.opposing_party as string}</span>}
          {c.jurisdiction   && <span><strong style={{ color: "#1C1917" }}>State:</strong> {c.jurisdiction as string}</span>}
          {c.court_name     && <span><strong style={{ color: "#1C1917" }}>Court:</strong> {c.court_name as string}</span>}
          {c.case_number    && <span><strong style={{ color: "#1C1917" }}>Case No.:</strong> {c.case_number as string}</span>}
        </div>
      </div>
      ...
      {timeline.length > 0 && (
        <section style={{ marginBottom: 32, pageBreakInside: "avoid" }}>
          <h2 style={{ fontSize: 14, textTransform: "uppercase", letterSpacing: "0.1em", borderBottom: "1px solid #E8E2D9", paddingBottom: 6, marginBottom: 12 }}>
            Timeline of Events ({timeline.length})
          </h2>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            {timeline.map((e, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #F2EDE5" }}>
                <td style={{ padding: "7px 0", verticalAlign: "top", width: 100, color: "#78716C", fontVariantNumeric: "tabular-nums" }}>{fmt(e.date)}</td>
                <td style={{ padding: "7px 0 7px 16px", verticalAlign: "top" }}>
                  {e.event as string}
                  {e.note ? <div style={{ fontSize: 12, color: "#78716C", marginTop: 3, fontStyle: "italic" }}>{e.note as string}</div> : null}
                </td>
              </tr>
            ))}
          </table>
            ...
            {evidence.map((e, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #F2EDE5" }}>
                ...
                <td style={{ padding: "7px 0 7px 12px", verticalAlign: "top" }}>
                  ...
                  {e.source_type ? <div style={{ fontSize: 12, color: "#78716C" }}>{e.source_type as string}</div> : null}
                  {e.summary     ? <div style={{ fontSize: 12, color: "#78716C", marginTop: 3 }}>{e.summary as string}</div> : null}
                </td>
              </tr>
            ))}
          </table>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            {documents.map((d, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #F2EDE5" }}>
                ...
                <td style={{ padding: "6px 0", textAlign: "right", color: "#78716C", width: 80 }}>{fmt(d.created_at)}</td>
      ...
      {finances.length > 0 && (
        <section style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: "flex", gap: 24, marginBottom: 16, fontSize: 13 }}>
            <div><span style={{ color: "#78716C" }}>Total assets: </span><strong>{money(totalAssets)}</strong></div>
            <div><span style={{ color: "#78716C" }}>Total debts: </span><strong style={{ color: "#DC2626" }}>{money(totalDebts)}</strong></div>
            <div><span style={{ color: "#78716C" }}>Net: </span><strong>{money(totalAssets - totalDebts)}</strong></div>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            {finances.map((f, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #F2EDE5" }}>
                <td style={{ padding: "6px 0", width: 80 }}>
                  <span style={{ fontSize: 11, padding: "2px 6px", borderRadius: 99, background: f.category === "Asset" ? "#DCFCE7" : f.category === "Debt" ? "#FEE2E2" : "#F2EDE5", color: f.category === "Asset" ? "#15803D" : f.category === "Debt" ? "#DC2626" : "#78716C" }}>
          ...
          {["todo", "inprogress", "done"].map(col => {
            ...
            return (
              <div key={col} style={{ marginBottom: 12 }}>
                <p style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#78716C", margin: "0 0 6px" }}>
                  {col === "todo" ? "To Do" : col === "inprogress" ? "In Progress" : "Done"}
                </p>
                {colTasks.map((t, i) => (
                  <div key={i} style={{ fontSize: 13, padding: "4px 0", display: "flex", gap: 8 }}>
                    <span>{col === "done" ? "✓" : "○"}</span>
                    <span style={{ textDecoration: col === "done" ? "line-through" : "none", color: col === "done" ? "#A8A29E" : "#1C1917" }}>{t.title as string}</span>
                  </div>
                ))}
              </div>
            );
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          {deadlines.filter(d => !d.completed).map((d, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #F2EDE5", fontSize: 13 }}>
              ...
              <span style={{ color: "#78716C", fontVariantNumeric: "tabular-nums" }}>{d.date as string}</span>
            </div>
          ))}
        </section>
      )}
      ...
      {/* Footer */}
      <div style={{ borderTop: "1px solid #E8E2D9", paddingTop: 16, marginTop: 40, fontSize: 11, color: "#A8A29E", textAlign: "center" }}>
        Generated by Vera · {generated} · This document is not legal advice and does not constitute an official court filing.
      </div>
    </div>
  );
}
